...
const userSettings = { theme: 'dark', notifications: true, language: 'ko' };
// 1. 객체를 문자열로 변환
let data = JSON.stringify( userSettings ) ;     // 임의의 자료를 문자열로 변환
// 2. 변환한 문자열을 localStorage 저장
localStorage.setItem( 'data' , data );            // 임의의 자료(value)를 속성(key)명 부여하여 저장
// * 확인
console.log( localStorage );
/*
...
// 2. JSON 타입으로 변환
let jsonData = JSON.parse( getDate );
// * 확인
