...
/*
예제 2: 매개변수를 사용하는 함수
임의의 두 개의 숫자(number)를 매개변수로 받아, 
그 합을 콘솔에 출력하는 printSum 함수를 정의하시오. printSum(5, 10)을 호출하여 결과를 확인하시오.
*/
// (1) 매개변수      : x , y  매개변수명은 아무거나.
// (2) 반환값        : X
// (3) 처리할 명령어  : 더하기 해서 console.log( );
function printSum( x , y ){ // 함수 정의
    console.log( x + y );   
}
printSum( 5 , 10 ); // 정의한 함수 호출 시 매개변수의 인수 전달 , x = 5 / y = 10
printSum( 6 , 2 );  // 8

/*
예제 3: 값을 반환(return)하는 함수
임의의 이름을 문자열(string)로 매개변수를 받아 
...
// (1) 매개변수      :  number
// (2) 반환값        :  
// (3) 처리할 명령어  : 1부터 n까지의 숫자를 콘솔에 차례대로 출력
function printNumbers(number){  // 함수 정의
    for( let n = 1 ; n <= number ; n++){    // n은 1부터 매개변수(number) 까지 1씩 증가
        console.log( n );
    }
}
printNumbers(7);
...
// (1) 매개변수      :  name
// (2) 반환값        :  
// (3) 처리할 명령어  : 매개변수(name)를 배열에(userList) .push( )
*/
...
예제 8: Boolean 값 반환하기
임의의 숫자 하나를 매개변수로 받아, 
그 숫자가 0보다 크면 true를, 그렇지 않으면 false를 반환하는 isPositive 함수를 만들어 보세요.
