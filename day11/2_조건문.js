...
//-------순서도-------
/* (1) 두 수를 입력받아 각 변수에 저장한다.     prompt , 변수
...
if( sum >= 90)                  // 1. 만약에(if) 총점(sum)이 90점 이상(>=) 이면
    {console.log("성공");}      // 2. (true) "성공" 출력
else
{ console.log("실패");}         // 3. (false) 아니면 "실패" 출력
*/
...
아이디가 'admin'이고 비밀번호가 '1234'와 모두 일치하면 '로그인 성공'을, 하나라도 다르다면 
'로그인 실패'를 출력하는 프로그램을 작성하시오.

*/
/*
// (1) 아이디와 비민번호 입력받아서 저장
let id =prompt("아이디 : ");
let pw =prompt("비밀번호 : ");

//let result = id == 'admin' && pw == '1234' ? "로그인 성공" : "로그인 실패" console.log(result)
// (2) 아이디와 비밀번호 일치하는지 확인
// (3) 모두 일치하면 '로그인 성공' 출력
// (4) 하나라도 다르면  '로그인 실패' 출력
if(id =='admin' && pw == '1234')
{console.log("로그인 성공");}
...
*/
// (1) 비밀번호 문자열로 입력받고 저장
// (2) 조건 : 길이 수에 따른 등급 매겨 출력
/*
let pw = prompt("비밀번호 입력 : ")                         
let pwLen = pw.length;                                       // - "문자열".length : 지정한 문자열의 문자개수 반환
if( pwLen >= 12 ){console.log("보안 등급 : 강함 ");}
else if(pwLen >= 8) {console.log(" 보안 등급 : 보통");}     // 12이상이 아니면 12 미만으로 가정       
else{console.log(" 보안 등급 : 약함(8자 이상으로 설정해주세요.)");}                      // 8이상이 아니면 8 미만으로 가정
*/
...
*/
/*
let seatList = [ 'O', 'X', 'O' ];  // (1) 배열이란? : 여러개의 자료들을 하나의 자료 [] 묶음
let seat = prompt("좌석번호 선택(0~1) :")  // (2) 인덱스 란? : 배열에 저장된 자료들의 순서번호 *0번시작*
// seat 번호== index 번호
if( seatList[seat]=='O') {// 만약에 좌석목록(seatlist)에서 선택(입력받은)한 (시트/인덱스)가 'o' 이면
    console.log('예약 불가');    
    // else if(seatList[seat] == 'x')
}else{
    console.log('예약 성공');
    seatList[seat] ='O'; 
}
...
*/
// 앞 조건에 이미 결과가 도출된다면 생략 가능한 조건들 : gamePoint && < 900 , gamePoint < 500
...
// 조건 : point >=90 , point >= 80 , point >=70 , point < 70
/*
let point = Number(prompt("점수 입력하세요 : "))
