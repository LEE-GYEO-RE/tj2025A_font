...
const 과목정보 = [
    ...
    { 과목코드: 2, 과목명: '머닝러신 입문', 교수자명: '최교수' },
    { 과목코드: 3, 과목명: '데이터 시각화', 교수자명: '박교수' }
];

const 수강신청내역정보 = [
    { 수강신청내역: 1, 학번: 1, 과목코드: 1, 성적: 'A+' },
    { 수강신청내역: 2, 학번: 1, 과목코드: 2, 성적: 'B' },
    { 수강신청내역: 3, 학번: 2, 과목코드: 2, 성적: 'A' },
    { 수강신청내역: 4, 학번: 2, 과목코드: 3, 성적: 'F' },
    { 수강신청내역: 5, 학번: 3, 과목코드: 1, 성적: 'C+' },
];
...
for (let i = 0; i <= 학생정보.length - 1; i++) {
    ...
    for (let j = 0; j <= 수강신청내역정보.length - 1; j++) {
        ...
        if (학생.학번 == 수강신청.학번) {
            ...
            for (let t = 0; t <= 과목정보.length - 1; t++) {
                let 과목 = 과목정보[t];
                if (과목.과목코드 == 수강신청과목) {
                    let 이수여부 = '';
                    if( 수강신청.성적 == 'F'){
                        이수여부 = `<span style = " color : red ">Fail</span>`;
                    }else{ 이수여부 = `<span style = " color : blue ">Pass</span>`;}
                    html += `${과목.과목명} (${과목.교수자명})
                    <ul> 
                        <li>성적 : ${수강신청.성적}</li>
                        <li>이수여부 : ${이수여부}</li>
