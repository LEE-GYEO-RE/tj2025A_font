...
function func5(){
    ...
    const remove = document.querySelector('#targetBox');
        console.log( remove );
    remove.remove('#targetBox'); 
    // .remove() =======> 마크업 자체 없앰
    // classList.remove ====> class만 없앰
...
function func7(){   // 함수 정의
    console.log('--- func7 exe ---')
    // 1. 'mainImage' 마크업 가져오기
...
function func8(){
    ...
    console.log(value);
    // onchage ---> 해당 마크업내 value가 변경 되었을때 발생
...
// 1. 전역배열 : 입력받은 값들을 저장하는 전역(함수 밖에) 배열
let array = [ ];
// 2. 버튼 클릭시 실행되는 함수 정의
