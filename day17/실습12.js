    ...
    [ 작업순서 ]
        ...
        3. (백) 함수(기능) 설계
            ...
            (2) '전체조회' 함수
                ...
                매개변수 : x
                반환값 : x
                로직 : 특정한 구역(table)에 모든 정보를 HTML 형식으로 출력한다.
                실행조건 :  1. 페이지 열렸을때(최초 1번)
                            2. 등록 성공했을때(새로고침)
...
let 코드자동번호 = 가계부목록[ 가계부목록.length-1 ].코드 // 현재 초기 배열의 마지막객체의 코드번호;
// [3] 기능(함수단위) 구현

// 1. 등록함수 정의 , 실행 : 등록버튼 onclick 했을때
function 등록함수(){ console.log('--- 등록함수 ---');                   // 2. 함수 onclick 확인
    // 3. input에 마크업 객체 3개 각각 가져오기
