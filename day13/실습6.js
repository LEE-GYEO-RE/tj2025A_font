...
// 메모 ( 기억 =변수 ) : 가장 큰 값
/*
let max = numbers[0]; // 첫번째 점수를 가장 큰 수로 기억/저장
// 만약에 max(기록) 보다 더 큰 수가 존재하면 더 큰수를 max에 (기록) 대입한다.
...
for 중첩 반복문을 사용하여 아래와 같은 모양의 별을 출력하시오.
    ...
    *           5               1               1 ~ ( 전체줄수 - 현재줄수 ) 0+1
    line은 1부터 5까지 1씩 증가             -> for( let line = 1 ; line <= 5 ; line++)
    star은 1부터 5/4/3/2/1 까지 1씩 증가    -> for( let star = 1 ; star <= 5-line+1 ; star++)
*/
...
*/
// 배열 내 -> 한 요소/값/문자열 -> '솔'이 포함. // 김하준(문자열)은 배열의 기반으로 만들어진 자료이다.
// (1) 모든 배열의 이름들을 하나씩 꺼낸다. < 배열 순회 >
/*
let userNames = ['김하준', '이서아', '박솔민', '최도윤'];  // 순서대로 꺼내서 확인해봐야 함
for( let index = 0; index <= userNames.length -1 ; index++){
    let name = userNames[index]; // index 번째의 이름 꺼낸다.
    console.log( name );
    // (2) 이름에 '솔'이 포함된
...
let 행3 = seatLayout[2] ;
    ...
    console.log( 행3[1]);
    console.log( 행3[2]);
    // for( let index = 0 ; index <= row.length -1 ; inedx++ ) { }
    // 행 하나에 열 추가가 나음
