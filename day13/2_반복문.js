...
*/
// * 배열 내 모든 정수 조회 : 0부터 마지막인덱스까지 조회
// for (let index = 0 ; index <= 마지막인덱스 ; index++){}
/*
let scores = [ 85, 92, 78, 65, 95 ];
let sum = 0; // 누적 총합계를 구해야 하므로 for문 밖에 선언
for ( let index = 0 ; index <= scores.length - 1 ; index++){
    // index는 0부터 마지막 인덱스 이하까지 1씩 증가 반복
    // let sum = 0; 여기에 합계 변수를 선언하면 안된다 : 합계변수는 누적해야하므로 선언을 반복하면 안됨.
    sum += scores[ index ];   // 배열내 index번째의 요소 값을 더한다.
    // 평균계산은 반복할 필요가 없다.
} // for end
...
for( let index = 0; index <= scores.length -1; index++){
    ...
    if( value == 100){ //만약에 해당 값이 100(만점) 이면
        ...
        break; // 가장 가까운 for{} 탈출 , 안써도 결과 같음, 그래도 break 쓰는게 효율적임. 찾는 순간 이후 코드 실행 안해도 되니깐.
    } // if end
} // for end
// break 만나면 [!!!여기!!!] 로 이동된다.
*/
...
let scores = [90, 45, 72, 88, 59, 100];
for( let index = 0; index <= scores.length -1; index++ ){
    let value = scores[index]; // index 번째의 요소 값 호출
...
/*
let bloodTypes = ['A', 'B', 'O', 'AB', 'A', 'B', 'A'];
let countA = 0; // A형의 개수를 저장하는 변수
for ( let index =0 ; index <= bloodTypes.length -1 ; index++){
    let blood = bloodTypes[index];  // idex번째의 혈액형 호출
...
for( let index =0; index <= nameArray.length -1 ; index++){
    ...
    let black = parseInt( score / 10); // 92/10 --> 9.2 -> parseInt(9.2) -> 9개
    for ( let circle = 1 ; circle <= 10; circle++){
        // 동그라미는 1부터 10이하까지 1씩증가 ( 총 10번 )
        if( circle <= black){ output += `<span>●</span>`} // 만일 현재 동그라미가 점수보다 적으면 검은색
        else{ output += `<span>○</span>`}                 // 크면 흰색
    }
    // -- 줄 바꿈
    output += `</div>`
} // for end
