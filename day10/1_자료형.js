...
// [문자열] : '' 또는 "" 묶음 자료
console.log(typeof "100") // 숫자도 "" 로 묶이면 문자열이다.
console.log(typeof "안녕하세요")
// 1. 이스케이프문자 : 기능이 포함된 특수문자 \백슬래시 : 키보드 원화기호
...
// 2. 템플릿 리터럴 
//      :`(백틱) 키보드 [tab] 키 위 , 변수나 식을 넣을 때는 ` ${} `
// 연산의 결과/반환은 항상 하나이다.
console.log("안녕" + "자바")        // +연결연산자 이용한 자료 합치지
console.log(`${"안녕"}${"자바"}`)   // ``묵음으로 자료 합치기
    // * 변수와 활용
let name = 'kim'
let classRoom = 204
...
// 방법2 (1) 백틱으로 묶는다. (2) ${} 안에 변수나 식을 넣는다. //방법2 주로 쓸거임
// `${변수명} 문자열 ${변수명} 문자열`
console.log(`${name}님, ${classRoom}호 강의실로 입장하세요.`)
...
console.log(채소상자)   // 변수에 null일 경우에는 null (자료가 존재 또는 유효 하지 않다.)

// [배열] : 하나의 변수에 값을 여러개 저장 , ,(쉼표)구분해서 대괄호[]로 묶는다.
let season = ["봄" , "여름" , "가을" , "겨울" ]
    // 인덱스 = 0   ,  1    ,    2   ,    3     , 길이/총개수 = 4
// - 나열 : 열거 하기 위해 하나씩 줄로 하는 것, - 열거 : 여러가지를 낱낱이 늘어놓음
...
console.log(season[4])  // undefined, 존재 하지 않은 인덱스 이므로
    // * [생각해보기1]
    //  변수개수 : 1개(season) , 리터럴 개수 : 4개("봄" , "여름" , "가을" , "겨울") 
    //  타입개수 : 2개( 배열, 문자열 )  , 변수가 갖는 자료의 개수 : 1개 ([])
    // ****** 변수 -> 배열 -> 문자열4개         ** 변수명[인덱스]
...
// 4. 배열 내 요소 추가 , 변수명.push(새로운 값) , 뒤에서부터 추가 된다.
season.push("서장훈")
console.log(season)     //['유재석' , '신동엽' , '서장훈']

// 5. 배열내 요소 삭제 , 변수명.splice(삭제할 인덱스번호 , 개수 )
season.splice(0 , 1)    // 0번 인덱스부터 1개 삭제
console.log(season)

// 6. 배열 내 특정 요소 찾가 , 변수명.indesOF(찾을값) , 
//    찾을 값이 배열내 존재하면 인덱스번호, 없으면 -1 반환
