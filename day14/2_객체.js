...
*/
const person = { name : '이겨레' , age : 28 , city : '부천' };
console.log( person );  // { name : '이겨레' , age : 28 , city : '부천' };
...
// 1. 객체 내 모든 값  (배열) 반환 : Object.values( 변수명 )
console.log( values ); // 배열 반환 : ['Cracking the Coding Interview', 'Gayle Laakmann McDowell', 500]
// 2. 반복문
for( let index = 0 ; index <= values.length -1 ; index++){
    console.log( values[index] ); // 배열 내 자료 반환
}
...
*/
person.email = 'rufp9801@gmail.com';
// 변수명.새로운 속성명 = 자료 ----> 추가
delete person.age;
// delete 변수명.삭제할 속성명 ----> 삭제
console.log( person );  // {name: '이겨레', city: '부천', email: 'rufp9801@gmail.com'}
...
// 특정한 속성의 자료만 호출 : 변수명['속성명']
console.log(user['job-title']); // 개발자
...
// 객체 내 특정한 속성 있는 지 확인. '찾을 속성명' in 변수명
const result = 'lever' in character ; 
console.log( result );      // false

/*
예제 8: Object.keys()와 Object.values()
country 객체의 모든 키(key) 들을 배열로 만들어 출력하고, 
모든 값(value) 들을 또 다른 배열로 만들어 콘솔에 각각 출력하시오.
const country = {
  name: '대한민국',
  capital: '서울',
  population: 51780000
...
const values1 = Object.values(country);
console.log( keys);
console.log( values1);
...
for( let index = 0 ; index <= names.length -1 ; index++){
    const key = names[index];               // index번째의 속성/key명 호출
    if ( inventory[key] < 10 ){              // index번째 속성/key 값이 10미만이면
        console.log( key );
    }
}
...
for ( let index = 0 ; index <= students.length -1 ; index++){
    const student = students[index];        // index번째 학생 정보 호출
...
[구현 조건]
  1. 공공데이터 포털에서 
    '인천광역시 부평구_인구 현황' 의 open API 신청하여 결과를 복사하여 'response' 변수에 저장하시오.
  let response = {}
  2. response 객체 안의 data 배열을 반복문을 사용하여 순회합니다.
  3. 각 동(행)의 정보를 표시할 HTML <table> 태그를 문자열로 만듭니다.
  4. 테이블의 각 셀에는 
    '동별', '세대수', '인구수(계)', '인구수(남)', '인구수(여)' 정보가 순서대로 포함되어야 합니다.
